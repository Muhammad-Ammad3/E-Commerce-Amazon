...
const cartSlice = createSlice({
  ...
  // cartSlice.js ke extraReducers wale part ko is tarah clean karein:
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCart.fulfilled, (state, action) => {
        // Agar backend se direct object mil rha hai `{ cart: { id: qty } }`
        const incomingCart = action.payload.cart;

        if (
          incomingCart &&
          typeof incomingCart === "object" &&
          !Array.isArray(incomingCart)
        ) {
          state.cartItems = incomingCart;
        } else if (Array.isArray(incomingCart)) {
          // Safe-side fallback agar array structure save ho gya ho
          const transformedCart = {};
          incomingCart.forEach((item) => {
            const id = item.productId || item.id || item._id;
            if (id) transformedCart[id] = item.quantity || 1;
          });
        ...
        // Total quantity recalculate karein
      ...
      // uploadCart fullfilled par status maintain rakhein taake loop na bane
